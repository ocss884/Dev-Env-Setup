...
- 推送仓库：`DOCKERHUB_USERNAME/my-dev-image`
- 定时任务：每天 `03:00 UTC`
...
构建镜像需要的文件都放在 [`docker/`](/Users/julian/py-project/my-dev-image/docker) 目录下。

直接编辑 [Dockerfile](/Users/julian/py-project/my-dev-image/docker/Dockerfile) 里的 `apt-get install` 列表即可。
...
- `image_name`：例如 `sglang-dev-tools`
...
docker build \
  ...
  -t your-dockerhub-name/my-dev-image:local ./docker
```
